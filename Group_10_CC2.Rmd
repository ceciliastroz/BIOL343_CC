...
---------|-------------|--------------
population_type | Is the population sexual or asexual | 
ovary_width | Width of ovaries | mm 
ovary_height | Height of ovaries | mm 
...
```{r import-clean}
# cc1_data <- 
data<-read.csv("./data/A1_Data_Excel1.csv")
...
$$ovary~width * ovary~height= ovary~area$$
$mm * mm = mm^2$
# Dynamic table
...
library(dplyr)
library(knitr)
kable(area_means, caption = "Ovary Area Means Summary Table")
```
...
library(ggplot2)
ggplot(cc1_data, aes(x=ovary_area))+geom_histogram(binwidth=2)+facet_wrap("population_type")+labs(x="Ovary Area (mm^2)", y="Frequency", title = "Ovary Area by Population Type")+theme_classic()
```
...
Source: 
[Wikimedia Commons](https://en.wikipedia.org/wiki/Solanum_pimpinellifolium#/media/File:Botanisk_tidsskrift_(17645191178).jpg)
# Repository
